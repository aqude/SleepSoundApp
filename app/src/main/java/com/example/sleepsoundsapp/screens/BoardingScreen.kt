...
fun BoardingScreen(navController: NavHostController) {
    ...
    val pagerState = rememberPagerState(pageCount = items.size, initialOffscreenLimit = 2, infiniteLoop = false, initialPage = 0)
    BoardingScreenPager(item = items, pagerState, modifier = Modifier.fillMaxSize().background(color = mainWallpaperColor).padding(horizontal = 24.dp))
}
